...
public class GetObjectFromApi {
	...
	@Autowired
	public GetObjectFromApi(RiotApiRequester riotApiRequester, MatchRepository matchRepository) {
	...
	public Match getMatch(RegionNation nation, String matchId) throws ParseException {
		...
		System.out.println(matchPlayerList);
	...
	private MatchPlayer mapToPlayer(Map map, String matchId) {
		...
		MatchPlayer player = MatchPlayer.builder()
				...
				.skinId(skinId)
				.species(species)
				//.matchPlayerChampionList(matchPlayerChampionList)
				//.matchPlayerTraitList(matchPlayerTraitList)
				.build();
	...
	public ApiMatch mapToApiMatch(Map map) {
